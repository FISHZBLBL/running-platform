...
import { getRunnerProfile, listRuns, listWeights } from "./_shared/data";
...
export default async function predictions(req: Request): Promise<Response> {
  try {
    ...
    const [runs, weights, runnerProfile] = await Promise.all([
      listRuns(username),
      listWeights(username),
      getRunnerProfile(username)
    ]);
    return json({
